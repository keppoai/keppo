...
2. Spawn 3 sub-agents with specialized personas using the Task tool
   ...
   - **Product UX Expert**: User experience, dashboard flows, error states, consistency, accessibility
   - **Security & Correctness Expert**: Auth bypasses, access control, logic bugs, data integrity, injection attacks
   - **Performance & Scalability Expert**: Query patterns, indexing, caching, resource usage, SaaS multi-tenancy bottlenecks
...
Parse the PR number and repo from the user's input. If the repo is not provided, try to infer it from the current git context:
...
Once you know `<OWNER/REPO>`, pass `--repo <OWNER/REPO>` to every `gh pr` and `gh api` call in this workflow. Never rely on the checked out repository context for PR lookups.
...
<role>
[Contents of references/<role>.md - e.g., product-ux-reviewer.md]
</role>
...
Severity levels:
- HIGH: Security vulnerabilities, access control bypasses, data loss risks, crashes, broken functionality
...
### Step 6: Determine Merge Verdict and Recommendation
...
**Only HIGH severity issues block merge.** MEDIUM issues are informational — they should be flagged but do not prevent merging.

Then determine the **recommendation** for the pr-watcher automation:

- **auto-fix**: All HIGH issues are mechanical/deterministic — an agent can fix them (code bugs, missing validation, broken tests, type errors)
...
- **ready**: No HIGH issues found — the PR is ready to merge
...
```markdown
<!-- pr-review:claude -->
## :mag: Code Review Summary (Claude)

**Verdict: [VERDICT EMOJI + TEXT]**
**Recommendation: [auto-fix | human-review | ready]**

Reviewed by 3 independent agents: Product UX Expert, Security & Correctness Expert, Performance & Scalability Expert.

### Issues Summary

| Severity               | File                  | Issue                  |
| ---------------------- | --------------------- | ---------------------- |
| :red_circle: HIGH      | `src/auth.ts:45`      | Auth bypass in workspace access |
  ...
  "comments": [
    {
      "path": "src/auth.ts",
      "line": 45,
      "body": "**:red_circle: HIGH** | security\n\n**Auth bypass in workspace access**\n\nDescription of the issue...\n\n:bulb: **Suggestion:** Add workspace membership check before granting access"
    }
  ]
}
```
...
- **HIGH**: Security vulnerabilities, access control bypasses, data loss risks, crashes, broken functionality, race conditions, UX blockers
- **MEDIUM**: Logic errors, unhandled edge cases, performance issues, sloppy code that hurts maintainability, poor error messages, missing loading/empty states, accessibility gaps, query patterns that won't scale
- **LOW**: Minor style issues, naming nitpicks, optional polish improvements

**Philosophy**: Security and access control issues are always HIGH. Sloppy code that hurts maintainability is MEDIUM, not LOW. Performance issues that affect SaaS scalability are MEDIUM or higher.

## File Structure

```
references/
  product-ux-reviewer.md           - Role description for the Product UX expert
  security-correctness-reviewer.md - Role description for the Security & Correctness expert
  performance-scalability-reviewer.md - Role description for the Performance & Scalability expert
