...
1. **Rebase onto the base branch:**
   ...
   - Determine the base branch: `gh pr view --json baseRefName --jq '.baseRefName'`
   - Determine the correct remote: if `git remote get-url upstream` succeeds, use `upstream/<base>`, otherwise use `origin/<base>`
   - Run: `git rebase <remote>/<base>`
   - If there are merge conflicts, resolve them: read each conflicting file, understand both sides, edit to resolve, `git add <file>`, `git rebase --continue` — repeat until complete
   - If the branch is already up to date, continue to the next step
   - Do not push or run checks yet — those happen in later steps

2. **Run `/keppo:pr-fix:comments`** to handle all unresolved review comments:
...
3. **Run `/keppo:pr-fix:actions`** to handle failing CI checks:
   - Fix failing tests (unit and E2E)
   - Fix lint, formatting, and type errors
   - Ensure all checks pass

4. **Run `/pr-push`** to commit and push all changes:
