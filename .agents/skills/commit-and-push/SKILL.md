...
description: Commit all working-tree and untracked changes in git, then push to the current branch using an automatically generated Conventional Commit message.
---

# Commit and Push

Use this skill when the user asks for a one-step git commit and push of current changes.
...
5. Pass the message to the script:
   ```bash
   bash .agents/skills/commit-and-push/scripts/commit_and_push.sh "<message>"
   ```
6. If upstream is not yet configured, the script handles `git push -u origin <branch>` automatically.
7. If the commit fails due to a pre-commit hook:
