...
- `/:orgSlug/:workspaceSlug/integrations/$provider` route:
  ...
  - runs tab shows a conversation-style card list with status icon, trigger type, relative time, duration, and a short triage summary instead of only status plus timestamp; clicking a run navigates to its detail route.
- `/:orgSlug/:workspaceSlug/automations/$automationId/runs/$runId` route:
  - header bar with back button, status badge, trigger type, duration, run ID, and cancel action.
  ...
  - Chat tab renders structured log events as styled bubbles:
    ...
    - `system`: inline operational updates with info icon; adjacent status lines can collapse into one system block.
