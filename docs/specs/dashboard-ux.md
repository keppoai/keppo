...
- `/:orgSlug/:workspaceSlug/integrations/$provider` route:
  ...
  - refreshable OAuth integrations keep the raw credential-expiry timestamp visible, but if that timestamp is already in the past and a refresh token is present the UI explicitly marks it as auto-refreshable instead of implying the connection is broken.
