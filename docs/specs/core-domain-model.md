...
- `automation_config_versions` stores trigger, runner, model, prompt, and network settings, but not per-automation AI key mode; execution mode is derived from org billing and runtime availability at save/run time: hosted bundled credits (shared across prompt generation and automation runtime) drive runtime when enabled, otherwise self-managed org keys fill the gap.
