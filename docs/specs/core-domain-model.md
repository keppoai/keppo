...
- `automation_config_versions` stores trigger, runner, model, prompt, and network settings, but not per-automation AI key mode; execution mode is derived from org billing and runtime availability at save/run time: hosted bundled credits drive runtime when enabled, otherwise self-managed org keys fill the gap.
