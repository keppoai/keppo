...
- `org_ai_keys` stores sandbox runner credentials for automation execution, including billing-managed bundled gateway keys that Keppo provisions and rotates for hosted bundled AI.
...
- `automation_runs` and `automation_trigger_events` are org-scoped records and persist required `org_id` values even when callers can also resolve tenant ownership through `workspace_id` or `automation_id`; this keeps org-indexed queries and maintenance paths consistent without relying on derived joins.
- `provider_webhook_deliveries` stores per-delivery provider webhook fanout state (`provider`, provider-issued `delivery_id`, matched org set, completed org subset) so retries only re-run the org ingestions that previously failed.
...
- `automations` stores operator-facing prose in `description`, an optional workflow diagram definition in `mermaid_content`, a mutable per-automation `memory` string capped at `20,000` characters, and the `mermaid_prompt_hash` that records which prompt revision last generated or manually aligned the diagram. Detail views derive Mermaid staleness by comparing the current config prompt hash to `mermaid_prompt_hash`.
- `automations.memory` is reused across future runs of the same automation rather than versioned with config snapshots. Dispatch injects non-empty memory into the runtime-owned system prompt inside `<memory>...</memory>` so the runner can reuse durable context from prior runs.
- `automation_config_versions` stores trigger, runner, model, prompt, and network settings, but not per-automation AI key mode; execution mode is derived from org billing and runtime availability at save/run time. When bundled gateway mode is enabled, clarifying questions, prompt generation, Mermaid regeneration, and automation runtime all meter against gateway spend and sync the derived remaining balance back into Convex. Without bundled gateway mode, prompt generation falls back to native fixed-credit deductions and runtime uses self-managed org keys.
...
- `ai_credits` and `ai_credit_purchases` track org-scoped AI entitlements. `ai_credit_purchases` stores purchased credit-pack rows with expiry, while `ai_credits` stores the included allowance, derived remaining purchased balance, and the last synced gateway spend/max-budget snapshot used to project the operator-visible remaining balance.
