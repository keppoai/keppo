...
The control-plane HTTP surface now runs through the unified TanStack Start runtime in `apps/web`: same-origin `/api/health*`, `/api/invites/*`, `/api/billing/*`, `/api/automations/generate-questions`, `/api/automations/generate-prompt`, the full `/api/automations/openai/*` helper/connect/callback family, `/api/mcp/test`, `/api/oauth/integrations/:provider/connect`, `/oauth/integrations/:provider/callback`, `/webhooks/:provider`, `GET|POST|DELETE /mcp/:workspaceId`, `/downloads/oauth-helper/{macos|windows}/latest`, `/internal/cron/maintenance`, `/internal/queue/dispatch-approved-action`, `/internal/health/deep`, `/internal/dlq*`, the full `/internal/automations/*` dispatch/terminate/log/complete family, and `/api/notifications/push/subscribe` routes plus typed app-internal server functions. Shared server helpers live under `apps/web/app/lib/server/api-runtime`; there is no separate first-party HTTP runtime.

## Public routes

| Area                       | Routes                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ------------------------- |
| Health                     | `GET /api/health`, `GET /api/health/deep`, `GET /api/health/flags`, `GET /api/health/audit-errors`, `GET /api/health/dlq`, `POST /api/health/dlq/:id/replay`, `POST /api/health/dlq/:id/abandon`, `GET /health`, `GET /health/deep`                                                                                                                                                                              |
...
| Automations                | `POST /api/automations/generate-questions`, `POST /api/automations/generate-prompt`, `GET /api/automations/openai/connect`, `GET /api/automations/openai/helper-session`, `POST /api/automations/openai/helper/callback`, `GET /api/automations/openai/helper-artifacts`, `GET /downloads/oauth-helper/{macos\|windows}/latest`, `GET /api/automations/openai/callback`, `POST /api/automations/openai/complete` |
...
- OpenAI helper-session routes are dashboard-authenticated and use short-lived signed helper tokens rather than dashboard cookies on the helper itself.
- Download routes for helper artifacts are configuration-driven redirect surfaces and may return `404 artifact_unavailable` when the operator has not configured hosting.
...
- `POST /api/invites/create`, `POST /api/invites/accept`, `POST /api/billing/*`, `POST /api/automations/generate-questions`, `POST /api/automations/generate-prompt`, `GET /api/automations/openai/connect`, `GET /api/automations/openai/helper-session`, `GET /api/automations/openai/helper-artifacts`, `POST /api/automations/openai/helper/callback`, `GET /api/automations/openai/callback`, `POST /api/automations/openai/complete`, `GET /downloads/oauth-helper/{macos|windows}/latest`, `GET|POST /internal/cron/maintenance`, `POST /internal/queue/dispatch-approved-action`, `GET /internal/health/deep`, `POST /internal/notifications/deliver`, `GET /internal/dlq`, `POST /internal/dlq/:id/replay`, `POST /internal/dlq/:id/abandon`, `GET /api/mcp/test`, and `POST /api/notifications/push/subscribe` are Start-owned routes executed directly inside `apps/web` with baseline API hardening headers applied in-process.
- `POST /api/oauth/integrations/:provider/connect` is Start-owned in `apps/web`. It resolves Better Auth session cookies in-process, enforces provider rollout and registry feature flags there, normalizes `return_to` to safe in-app paths, and returns the provider `oauth_start_url`.
- `GET /oauth/integrations/:provider/callback` is Start-owned in `apps/web`. It verifies signed OAuth state, enforces provider rollout and callback idempotency, exchanges provider credentials in-process, persists the resulting org integration, and redirects back into the dashboard.
- `POST /webhooks/:provider` is Start-owned in `apps/web`. It resolves canonical webhook-capable providers in-process, verifies provider signatures before any state mutation, enforces provider rollout and registry feature flags there, and records delivery dedupe/automation-trigger fanout.
...
- The production maintenance sweep is driven by Convex cron job `maintenance-sweep` (`cron_heartbeats:scheduledMaintenanceSweepWithHeartbeat`) every 2 minutes; `POST /internal/cron/maintenance` invokes the same wrapper manually for operators and tests.
...
- Automation-key OpenAI OAuth connect/callback uses Keppo-owned PKCE + signed-expiring state and does not reuse provider-integration OAuth records or third-party client IDs.
- Helper-session OpenAI OAuth adds a second trusted ingest path: Keppo signs a short-lived helper token bound to org/user/session context, the desktop helper submits only that token plus the captured localhost callback URL, and API dedupe keys prevent replay of the same helper callback submission.
