...
- OpenAI connect/complete routes are dashboard-authenticated and never expose refreshable OAuth credentials to the browser.
...
- `POST /api/invites/create`, `POST /api/invites/accept`, `POST /api/billing/*`, `POST /api/automations/generate-questions`, `POST /api/automations/generate-prompt`, `GET /api/automations/openai/connect`, `GET /api/automations/openai/callback`, `POST /api/automations/openai/complete`, `GET|POST /internal/cron/maintenance`, `POST /internal/queue/dispatch-approved-action`, `GET /internal/health/deep`, `POST /internal/notifications/deliver`, `GET /internal/dlq`, `POST /internal/dlq/:id/replay`, `POST /internal/dlq/:id/abandon`, `GET /api/mcp/test`, and `POST /api/notifications/push/subscribe` are Start-owned routes executed directly inside `apps/web` with baseline API hardening headers applied in-process.
...
- OpenAI localhost callback completion keeps the browser flow and credential exchange split: the browser returns a signed `state` to the loopback listener, and only the API runtime exchanges the pasted callback URL for refreshable credentials.
