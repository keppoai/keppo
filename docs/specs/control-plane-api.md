...
- `POST /webhooks/:provider` is Start-owned in `apps/web`. It resolves canonical webhook-capable providers in-process, verifies provider signatures before any state mutation, enforces provider rollout and registry feature flags there, records delivery dedupe plus per-org webhook fanout state, and only returns `200` after every still-pending matched-org ingestion succeeds so provider retries remain available for the specific orgs that previously failed.
...
- The automation builder now uses a two-call flow:
  - `POST /api/automations/generate-questions` returns up to 4 structured clarification questions using only `radio`, `checkbox`, and single-line `text` inputs. In hosted bundled mode it provisions bundled gateway credentials on demand, syncs the pre/post generation balance from gateway spend, may return `402 ai_credit_limit_reached` before generation, and returns explicit billing metadata with `charged_credits` plus optional `charged_budget_usd` and `remaining_credits`. Without bundled gateway mode, the question stage remains free and returns informational billing metadata only.
  - `POST /api/automations/generate-prompt` accepts the original `user_description` plus optional `clarification_questions[]` and `clarification_answers[]` and returns the generated draft with `credit_balance` plus explicit draft billing metadata. In hosted bundled mode, final draft generation and Mermaid-only regeneration use the same pre/post gateway-spend sync model as the question stage. Without bundled gateway mode, draft generation and Mermaid regeneration fall back to the legacy native AI-credit ledger and deduct 1 visible credit each.
