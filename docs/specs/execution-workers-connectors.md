...
- Shared provider base classes now own the repeated connector and SDK scaffolding:
  ...
  - Provider entrypoints stay stable at `connector.ts`, `real.ts`, and `fake.ts`, but the large provider-specific implementations may live in adjacent runtime files (for example `connector-runtime.ts`, `sdk-runtime.ts`, `fake-client-runtime.ts`) so imports stay stable while the scanned entry files remain thin.
...
- Dispatch path requirements:
  ...
  - derive stable hashed OpenAI trace identifiers/group ids server-side and pass them into the sandbox runner together with optional trace-export configuration only when a dedicated tracing key is configured.
...
- `automation_scheduler_node:reconcileProviderTriggerSubscriptions` is the provider-trigger maintenance entrypoint. It loads active provider-trigger configs, refreshes provider-owned lifecycle state, renews Gmail watches when configured, advances polling cursors, and ingests normalized provider events into Convex. Hosted preview intentionally does not register the `automation-provider-trigger-reconcile` cron wrapper, and preview cron-health expectations omit that job for the same reason, so this work only progresses there when the action is invoked directly for explicit validation.
