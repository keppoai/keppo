...
| `KEPPO_CODE_MODE_SANDBOX_PROVIDER`                         | server         | `docker`                          | Code Mode sandbox. `jslite` is available only for local/dev experimentation; use `vercel` or `unikraft` for non-local execution.                                                                                                 |
...
| `KEPPO_JSLITE_PROJECT_PATH`                                | server         | `../jslite` (runtime default)     | Optional local/dev-only path to the JSLite checkout used when `KEPPO_CODE_MODE_SANDBOX_PROVIDER=jslite`.                                                                                                                         |
| `KEPPO_JSLITE_SIDECAR_PATH`                                | server         | -                                 | Optional local/dev-only explicit path to a built `jslite-sidecar` binary. Overrides `KEPPO_JSLITE_PROJECT_PATH`.                                                                                                                 |
...
- Local `jslite` Code Mode execution requires a prebuilt `jslite-sidecar` binary. Either build it yourself from `../jslite` or `KEPPO_JSLITE_PROJECT_PATH` with a Rust toolchain so Keppo can discover `target/{release,debug}/jslite-sidecar`, or point `KEPPO_JSLITE_SIDECAR_PATH` at an already-built binary. Keppo rejects `KEPPO_CODE_MODE_SANDBOX_PROVIDER=jslite` in strict/non-local deployments; see [`JSLITE_BLOCKERS.md`](../JSLITE_BLOCKERS.md).
