...
| `KEPPO_OPENAI_TRACING_ENDPOINT`                            | server         | -                                 | Optional override for the OpenAI trace-export endpoint (e.g. `https://api.openai.com/v1/traces/ingest`). When unset, defaults to empty (disabled). Useful only when you intentionally proxy or test trace ingestion separately from normal model traffic.                                                             |
