...
| `KEPPO_API_INTERNAL_BASE_URL`                              | server, convex | auto                              | Base URL for Convex-to-API internal calls. Derives `${KEPPO_URL}/api` and must be reachable from automation sandboxes for signed log, completion, and session-artifact callbacks.                                                 |
| `KEPPO_MASTER_KEY_INTEGRATION`                             | server         | —                                 | Optional integration-specific KEK                                                                                                                                                                                                 |
| **LLM gateway**                                            |                |                                   |                                                                                                                                                                                                                                   |
| `KEPPO_LLM_GATEWAY_URL`                                    | server         | -                                 | Dyad Gateway base URL for bundled AI runtime. When set, bundled runtime is enabled and BYO authoring falls back to a fail-closed unavailable state instead of exposing a mixed mode. Required when `KEPPO_STRICT_MODE` is truthy. |
