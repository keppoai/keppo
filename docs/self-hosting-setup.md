...
- Local `jslite` Code Mode execution requires either a built `jslite-sidecar` binary or a `../jslite` checkout with a usable Rust toolchain. Keppo rejects `KEPPO_CODE_MODE_SANDBOX_PROVIDER=jslite` in strict/non-local deployments; see [`JSLITE_BLOCKERS.md`](../JSLITE_BLOCKERS.md).
