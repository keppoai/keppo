...
| **Sandbox**                                                |                |                                   |                                                                                                                                                                                                                                   |
| `KEPPO_SANDBOX_PROVIDER`                                   | server         | `docker`                          | Automation sandbox. Set `vercel`, `fly`, or `unikraft` for production-tier remote execution.                                                                                                                                      |
...
| `FLY_AUTOMATION_IMAGE`                                     | server         | `registry-1.docker.io/library/node:22-bookworm` | Optional base image for Fly automation machines. Keep Node and npm available, or the repo-owned runner bootstrap will fail.                                                                                         |
| `FLY_AUTOMATION_MACHINE_REGION`                            | server         | auto                              | Optional Fly region override for automation machines.                                                                                                                                                                              |
...
| `KEPPO_FLY_ALLOW_UNENFORCED_MCP_ONLY`                      | server         | `false`                           | Required when `KEPPO_SANDBOX_PROVIDER=fly`. Explicit operator opt-in acknowledging that Fly does not currently enforce Vercel-style outbound allowlists for `mcp_only`, so startup now fails closed until the operator accepts that contract. |
...
- Fly automation machines bootstrap the repo-owned runner inside the guest at launch time. The wrapper keeps `npm install` in a tightly allowlisted bootstrap env, covers the full install-plus-run lifecycle with the sandbox timeout, runs callback posts with explicit timeouts, and gives up on log-drain retries after a bounded number of failures so terminal completion still posts. If you override `FLY_AUTOMATION_IMAGE`, keep a compatible Node + npm runtime available or machine startup will fail before the automation can report completion.
- Fly does not currently enforce instance-level outbound allowlists. Set `KEPPO_FLY_ALLOW_UNENFORCED_MCP_ONLY=true` whenever `KEPPO_SANDBOX_PROVIDER=fly` so startup fails closed until the operator explicitly accepts that `mcp_only` remains a runner/tooling boundary rather than a hard egress boundary there.
...
6. Set `KEPPO_FLY_ALLOW_UNENFORCED_MCP_ONLY=true` to acknowledge the current lack of hard egress enforcement on Fly; Keppo now fails startup closed for `KEPPO_SANDBOX_PROVIDER=fly` until that acknowledgment is explicit.
