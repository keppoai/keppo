...
| `KEPPO_OPENAI_TRACING_API_KEY`                             | server         | `OPENAI_API_KEY`                  | Optional dedicated key for exporting automation traces through the OpenAI tracing API. When unset, Keppo falls back to `OPENAI_API_KEY`; `KEPPO_E2E_MODE` disables trace export by default.                                       |
