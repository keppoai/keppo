...
- **adjacent `../jslite` checkout or explicit `KEPPO_JSLITE_PROJECT_PATH` with a usable Rust toolchain, or explicit `KEPPO_JSLITE_SIDECAR_PATH` to a built sidecar** (required only when intentionally validating `KEPPO_CODE_MODE_SANDBOX_PROVIDER=jslite`; this mode is local/dev-only and stays blocked in production)
