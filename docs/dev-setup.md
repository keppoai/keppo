...
- **prebuilt `jslite-sidecar` binary** (required only when intentionally validating `KEPPO_CODE_MODE_SANDBOX_PROVIDER=jslite`; either build it yourself from an adjacent `../jslite` checkout or explicit `KEPPO_JSLITE_PROJECT_PATH` with a Rust toolchain so Keppo can discover `target/{release,debug}/jslite-sidecar`, or point `KEPPO_JSLITE_SIDECAR_PATH` at an already-built binary; this mode is local/dev-only and stays blocked in production)
