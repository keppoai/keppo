...
- Use `ubicloud-standard-2` for other workflows that orchestrate, gate, or accompany E2E (e.g. `ci-pr.yml` check/result jobs, `get-main-to-green.yml`, `issue-agent-issue-to-pr.yml`). `fix-pr.yml` uses `namespace-profile-linux-large` for its main job.
