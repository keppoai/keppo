...
- In `e2e-shared.yml`, use `ubuntu-latest` for the Checks and Report jobs; use `ubicloud-standard-4` for Local Convex and Playwright E2E shard jobs. Use `ubicloud-standard-4` for `e2e-meta.yml` and `claude-deflake-e2e.yml`.
