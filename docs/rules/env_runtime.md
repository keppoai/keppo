...
- Hosted `staging` and `production` Convex env sync must start from the selected repo-root env file but preserve non-empty deployment-provided overrides and secrets from the shell environment. Do not drop deployment-only values such as `VERCEL_AUTOMATION_BYPASS_SECRET` when pushing hosted Convex env.
...
- MCP built-in tool schemas that feed hosted runners must preserve known client-compatible argument aliases (for example Codex may call `search_tools` with `q` instead of `query`). Tighten those schemas only with paired API and targeted E2E coverage plus an intentional client migration plan.
