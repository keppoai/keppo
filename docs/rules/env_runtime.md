...
- Hosted preview Convex deployments must not register the high-cost `maintenance-sweep` or `automation-provider-trigger-reconcile` cron jobs. Keep that preview gate at cron-registration time in `convex/crons.ts` so manual maintenance and direct provider-trigger reconciliation still work when operators intentionally invoke them.
- Because hosted preview cron registration is gated inside Convex runtime code, `KEPPO_ENVIRONMENT` must stay in the managed hosted Convex env manifest so preview/staging/production sync the same environment signal into Convex before module evaluation.
