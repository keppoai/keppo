...
- Fake OpenAI Responses repro harnesses are the inverse case: when `KEPPO_E2E_OPENAI_RESPONSES_FAKE=1`, the wrapper that boots local Convex must clear `KEPPO_LLM_GATEWAY_URL` before syncing env so bundled runtime stays disabled and BYOK-only response-path tests do not silently render hosted AI mode.
...
- Any host-side test or fake service that Docker sandboxes call through `host.docker.internal` must listen on `0.0.0.0`, not only `127.0.0.1`; loopback-only listeners are reachable from the host process but not from the sandbox container.
