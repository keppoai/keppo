...
- Notification-preference endpoint persistence checks should scope backend helper queries to the seeded user when that identity is known; org-wide endpoint scans are slower under CI load and can miss the intended row while the optimistic UI is already visible.
