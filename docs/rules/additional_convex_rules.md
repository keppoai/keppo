...
- When forwarding optional Convex args through API or dashboard adapters, do not gate them on truthiness if empty strings or `false` are meaningful values. Use explicit `hasOwnProperty`/`!== undefined` checks so clear-value mutations survive the client wrapper.
...
- Do not issue per-row secondary lookups inside list queries when the rows share a parent key. Prefer one parent-scoped indexed read plus in-memory mapping (or denormalize the field) over N+1 `by_custom_id` queries.
