...
- Do not reuse GitHub App installation tokens minted before a long agent run for deterministic post-agent steps. Mint a fresh scoped token after the agent step for post-agent checkouts, comments, thread actions, label updates, and other privileged GitHub mutations.
