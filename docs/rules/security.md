...
- The `jslite` Code Mode sandbox provider is also local-dev only. It may use a host-side sidecar process for development experiments, but production-tier Code Mode execution must continue to use `vercel` or `unikraft` until `JSLITE_BLOCKERS.md` is resolved.
