...
- Repository security advisories in `triage` or `draft` are embargoed data. Do not send their summaries or descriptions to external model providers; keep unpublished-advisory dedupe local or limit model prompts to already published advisories.
