...
- Retry-safe dispatch claim reuse must still have a bounded failure budget. If the dispatch endpoint keeps rejecting a pending run (for example `404 run_not_found`), stop redispatching after a small bounded number of attempts and move the run to a terminal failure state with an operator-readable reason.
...
- The Docker sandbox provider is local-dev only. Both automation and code-mode sandbox factories must throw at construction time when `docker` mode is selected outside a local-dev environment (`NODE_ENV=development|test` or `KEPPO_E2E_MODE=true`). Production deployments must use the `vercel`, `fly`, or `unikraft` sandbox provider.
- Sandboxes should receive the minimum env and network access needed to bootstrap and execute.
- When a remote sandbox must install packages or dependencies before the runner starts, perform that install step in a secret-free or tightly whitelisted environment and pass the full runtime env only to the final runner process.
