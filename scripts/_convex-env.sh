...
setup_e2e_convex_env() {
  ...
  if [ "${KEPPO_E2E_OPENAI_RESPONSES_FAKE:-}" = "1" ]; then
    # `convex env set --from-file` only overwrites keys present in the batch.
    # Fake Responses repro runs must explicitly clear the gateway key so a
    # previous local value does not keep bundled runtime mode enabled.
    append_convex_env_value "$input_file" "KEPPO_LLM_GATEWAY_URL" ""
    append_convex_env_value \
      "$input_file" \
      "KEPPO_E2E_OPENAI_BASE_URL" \
      "${KEPPO_E2E_OPENAI_BASE_URL:-${KEPPO_FAKE_EXTERNAL_BASE_URL}}"
  else
    append_convex_env_value "$input_file" "KEPPO_LLM_GATEWAY_URL" "${KEPPO_LLM_GATEWAY_URL-}"
  fi
