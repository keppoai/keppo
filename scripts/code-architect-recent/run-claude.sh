...
if npx -y "@anthropic-ai/claude-code@${CLAUDE_CODE_VERSION}" \
  ...
  --allowedTools \
  Read \
  "Write(./out-code-architect/**)" \
  "Edit(./out-code-architect/**)" \
  "MultiEdit(./out-code-architect/**)" \
  Glob \
  Grep \
  LS \
  "Bash(cat:*)" \
  ...
  "Bash(head:*)" \
  "Bash(ls:*)" \
  "Bash(node .workflow-base/.agents/skills/code-architect-recent/scripts/select_recent_files.mjs:*)" \
  "Bash(pwd:*)" \
  "Bash(rg:*)" \
  "Bash(sed:*)" \
  "Bash(sort:*)" \
