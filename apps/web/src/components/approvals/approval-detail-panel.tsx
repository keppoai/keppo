    ...
    <ScrollArea className="h-full">
      <div className="flex flex-col gap-6 p-6">
            ...
            {payloadSummary.length > 0 ? (
              <dl className="mt-4 grid gap-3 md:grid-cols-2">
                {payloadSummary.map((item) => (
                  <div key={item.label} className="rounded-xl bg-background px-3 py-3">
        ...
        <div className="flex flex-col gap-2">
          <h4 className="text-sm font-medium">Decision Trace</h4>
          {celMatches.length > 0 && (
            <div className="rounded-md border p-3 text-sm">
              <div className="mb-2 font-medium">CEL Rule Matches</div>
              {celMatches.map((match) => (
                <div key={match.id} className="mb-2 last:mb-0">
                  <Badge variant={match.effect === "deny" ? "destructive" : "secondary"}>
                    {match.effect}
                  </Badge>
                  <div className="mt-1 font-mono text-xs">{match.expression_snapshot}</div>
                </div>
              ))}
            </div>
          )}
          {policyDecisions.length > 0 && (
            <div className="rounded-md border p-3 text-sm">
              <div className="mb-2 font-medium">Policy Decisions</div>
              {policyDecisions.map((decision) => (
                <div key={decision.id} className="mb-2 last:mb-0">
                  ...
                  <div className="mt-1">{decision.explanation}</div>
          ...
          <div className="rounded-md border p-3 text-sm">
            <div className="mb-2 font-medium">Approval History</div>
            {approvals.length === 0 && (
              <p className="text-muted-foreground">No approvals recorded.</p>
            )}
            {approvals.map((entry) => (
              <div key={entry.id} className="mb-2 last:mb-0">
                ...
                {entry.reason && <div className="mt-1">{entry.reason}</div>}
                <div className="text-xs text-muted-foreground">
          ...
          <div className="rounded-md border p-3 text-sm">
            <div className="mb-2 font-medium">Execution Timeline</div>
            {timeline.length === 0 && (
            ...
            {timeline.map((event) => (
              <div key={event.id} className="mb-2 last:mb-0">
                ...
                <div className="mt-1 font-mono text-xs">{pretty(event.payload ?? {})}</div>
              </div>
            ))}
          </div>
        </div>
