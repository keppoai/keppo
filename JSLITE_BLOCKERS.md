...
2. Keppo does not ship a pinned JSLite artifact today. The provider currently resolves a prebuilt sidecar binary from `KEPPO_JSLITE_PROJECT_PATH`, the adjacent `../jslite` default checkout, or an explicit `KEPPO_JSLITE_SIDECAR_PATH`, which is workable for local development but not a reproducible hosted deployment contract.
